  ...
  return myList.length === 0 ? (
    ...
    >
      <div className="flex items-center justify-center w-full h-full flex-col">
        <BsPlusCircle
          style={{ width: "120px", height: "120px", color: "#4E515E" }}
        />
        <h3 className=" py-8 pb-2 text-2xl text-[#f9f9f9]">
          Mi lista está vacía.
        </h3>
        <p className="text-[#f9f9f9] text-[15px] py-2 pb-2 ">
          El contenido que agregues aparecerá aquí.
        </p>
      </div>
    </div>
  ) : (
    ...
    >
      ...
      <div className="grid grid-cols-4 gap-5">
        {myList.map((movie) => (
          <CardMovie key={movie.id} movie={movie} isInList={true} />
        ))}
      </div>
    </div>
  );
